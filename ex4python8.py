    ...
    with open('Python_08.codons-frame-6.nt', 'w') as wFile:
        for key in seq_dict:
            ...
            for j in [0, 1, 2]:
                ...
                # Escreve no arquivo
                wFile.write(f"{key} {codon_frame_key}\n{split_seq}\n")
                wFile.write(f"{key} {rc_codon_frame_key}\n{rc_split_seq}\n")

    # Testa a tradução de um exemplo
    test = 'GCT CAG CTC'
    print(test)
    print(translate(test))  # Função de tradução para aminoácidos

    # Abre o arquivo para gravar as traduções
    with open('Python_08.translated.aa', 'w') as protFile:
        for key in total_nt_dict:
            # Traduz cada sequência de códons para aminoácidos
            for frame in total_nt_dict[key]:
                seq_to_trans = total_nt_dict[key][frame]
                prot_seq = translate(seq_to_trans)  # Converte para sequência de aminoácidos
                protFile.write(f"{key} {frame} translated\n{prot_seq}\n")

# Se não fornecer o arquivo como argumento, avisa ao usuário
except IndexError:
    print('Por favor, forneça um arquivo como argumento.')
